...
def run_on_data(i, method, structure, n_samples, files_input_name, verbose):
    ...
    # tp.plot_timeseries(data_tigramite, figsize=(15, 5));
    # plt.show()
    ...
    if method == "GrangerPW":
        ...
        model = cd.TPCMCITMI(nodes, sig_level=0.05, nlags=5)
        model.infer_from_data(data)
    elif method == "PCTCE":
        model = cd.PCTCE(nodes, sig_level=0.05, nlags=3)
        model.infer_from_data(data)
    elif method == "NBCB_pw":
        ...
        model = cd.FCITCE(nodes, sig_level=0.05, nlags=5)
        model.infer_from_data(data)
    elif method == "FCITMI":
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 5:
        ...
        method = "FCITCE"  # GrangerPW, GrangerMV, TCDF, PCMCICMIknn, PCMCIParCorr, oCSE, PCTMI, tsFCI, FCITMI, VarLiNGAM, TiMINO, TsKIKO, Dynotears
        structure = "7ts2h"
