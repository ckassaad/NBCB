...
def get_ground_truth_temperature(nodes):
    gtrue = nx.DiGraph()
    ...
    ogtrue.add_edges_from([(nodes[1], nodes[0])])
...
def run_on_data(method, verbose):
    ...
    nodes = [''.join(x.split()) for x in nodes]
    gtrue, ogtrue, sgtrue = get_ground_truth_temperature(nodes)
    ...
    if save_model:
        nx.write_gpickle(model.ghat, "./experiments/graphs/summary_other_and_self_graphs/temperature/"+method)
        nx.write_gpickle(model.oghat, "./experiments/graphs/summary_other_graphs/temperature/"+method)
        nx.write_gpickle(model.sghat, "./experiments/graphs/summary_self_graphs/temperature/"+method)
...
if __name__ == "__main__":
    ...
    with open(
            "./experiments/performance_average/summary_other_performance_average/" + str(method) + "_temperature",
            "w+") as file:
    ...
    with open(
            "./experiments/performance_average/summary_self_performance_average/" + str(method) + "_temperature",
            "w+") as file:
