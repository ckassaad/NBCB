...
from baselines.scripts_python.python_packages.CITMI.ctmi_new import i_ctmi, ctmi, align_matrix, tmi, window_size, gamma_matrix_window_matrix, self_ctmi, indep_test_iid
# from gctmi import gctmi

from baselines.scripts_python.python_packages.CITMI.ctmi_new import indep_test

from baselines.scripts_python.python_packages.CITMI.tigramite.tigramite.pcmci import PCMCI
from baselines.scripts_python.python_packages.tigramite.tigramite import data_processing as pp
from baselines.scripts_python.python_packages.tigramite.tigramite.independence_tests import CMIknn
        ...
        for rs in r_list:
            ...
            for r in rs:
                ...
                if self.adaptive_window:
                    # select and drop NA
                    # z[self.names[r]] = self.series[self.names[r]].dropna()
                    z[self.names[r]] = window_representation(self.series[self.names[r]], windows_size=3)
                    if z[self.names[r]].shape[0] < n_z_min:
                        n_z_min = z[self.names[r]].shape[0]
                else:
                    z[self.names[r]] = self.data_dict[self.names[r]]
            if self.adaptive_window:
                for r in rs:
                    z[self.names[r]] = z[self.names[r]].loc[:n_z_min]
...
class PCTMI(CITMI):
    ...
    def rule_origin_causality(self):
        ...
        for p in range(self.d):
            for q in range(self.d):
                if (p != q) and (self.graph.edges[p, q] ==0):
    ...
    def rule_origin_causality_mi_test(self):
        """
        rule 0 (origin of causality) from PC
        """
        if self.verbose:
    ...
    def test_if_part_of_sheilded_triple(self, i ,j):
        r_list = [r for r in range(self.d) if (self.graph.edges[i, r] != 0) and (self.graph.edges[i, r] != 0)]
        if len(r_list)>0:
            return True
        else:
            return False

    def _find_all_open_backdoor_paths_util(self, pp, q, visited, path, all_path, e):
        """
        sub function of _find_shortest_directed_paths
        :param p: index of time series
        :param q: index of time series
        ...
        if pp == q:
            ...
            # If current vertex is not destination
            # Recur for all the vertices child of this vertex
            child_p = np.where(self.graph.edges[pp, :] != 0)[0]
            # child_p = np.delete(child_p, np.where(child_p == pp))
            for k in child_p:
                if not visited[k]:
                    if self.graph.edges[k, pp] == 2:
                        e_new= -1
                    elif self.graph.edges[pp, k] == 2:
                        e_new = 1
                    else:
                        e_new = 2
                    # e= e_new
                    test = (e == 1) and (e_new == -1)
                    if not test:
                        self._find_all_open_backdoor_paths_util(k, q, visited, path, all_path, e_new)

        # Remove current vertex from path[] and mark it as unvisited
        path.pop()
        visited[pp] = False

    def _find_all_open_backdoor_paths(self, p, q):
        """
        find shortest directed path between time series of index p and time series of index q
        :param p: index of time series
        :param q: index of time series
        ...
        # Call the recursive helper function to print all paths
        e = 2   # 1: ortiented p-> q  ; -1: oriented q->p   ; 2: unoriented
        self._find_all_open_backdoor_paths_util(p, q, visited, path, all_path, e)
        return all_path
    ...
    def rule_entropy_reduction_gamma(self):
        ...
        for i in range(self.graph.d):
            for j in range(i + 1, self.graph.d):
                if (self.graph.edges[i, j] == 1) and (self.graph.edges[j, i] == 1):
                    path_spurious_correlations = self._find_all_open_backdoor_paths(i, j)
                    if len(path_spurious_correlations) == 0:
                        gij = self.gamma_matrix[self.names[j]].loc[self.names[i]]
                        if gij > 0:
                        ...
                        if test_parents:
                            if self.window_matrix[self.names[j]].loc[self.names[i]] < self.window_matrix[self.names[i]].loc[self.names[j]]:
                                if self.verbose:
                                    ...
                                    print("=> orient " + str(i) + "-> " + str(j))
                                self.graph.edges[i, j] = 2
                            elif self.window_matrix[self.names[j]].loc[self.names[i]] > self.window_matrix[self.names[i]].loc[self.names[j]]:
                                if self.verbose:
                                    print(str(i) + "-" + str(j) + "g(i,j)=0 and w(i,j)> w(j,i)", end=" ")
        ...
        for j in range(self.d):
            ...
            for i in range(self.d):
                if self.graph.edges[i, j] != 0:
                    if self.graph.edges[j, i] != 2:
                        for t in range(5):
                            if t == 0:
                                if i != j:
                                    links_dict[j].append((i, t))
                            else:
                                    links_dict[j].append((i, -t))

        data_tigramite = pp.DataFrame(self.series.values, var_names=self.series.columns)
        pcmci = PCMCI(
            dataframe=data_tigramite,
            cond_ind_test=CMIknn(),
            verbosity=1)
        pcmci.run_pcmci(selected_links=links_dict, tau_min=0, tau_max=5, pc_alpha=0.05)
...
class TPCTMI(PCTMI):
            ...
            for j in range(self.d):
                if self.adaptive_window:
                    if (self.graph.edges[i, j] == 2) and (self.graph.edges[j, i] == 1):
                        lag_gamma.append(abs(self.gamma_matrix[self.names[j]].loc[self.names[i]]) + self.window_matrix[self.names[i]].loc[self.names[j]])
                    elif (self.graph.edges[i, j] == 1) and (self.graph.edges[j, i] == 2):
                        lag_gamma.append(abs(self.gamma_matrix[self.names[i]].loc[self.names[j]]) + self.window_matrix[self.names[j]].loc[self.names[i]])
            ...
            for p, q, r_set, cmi in zip(ranks.elem_p, ranks.elem_q, ranks.elem_r, ranks.val):
                ...
                if test:
                    ...
                    if self.verbose:
                        ...
                        #     cmi) + "; I(p,q)=" + "{: 0.5f}".format(mi), end=" ")
                        print("p=" + self.ts_names[p] + "; q=" + self.ts_names[q] + "; r=", end=" ")
                        for r in r_set:
                            print(self.ts_names[r],  end=" ")
                        print("; I(p,q|r)=" + "{: 0.5f}".format(
                            cmi) + "; I(p,q)=" + "{: 0.5f}".format(mi), end=" ")
    ...
    def ts_remove_hom_edges(self, p, q, p_name, q_name):
        if (p < self.tgraph.d) and (q < self.tgraph.d) and (self.ts_names_dict_inv[self.ts_names[p]] == p_name) and \
            ...
            if self.verbose:
                print("=> remove link between " + self.ts_names[p] + " and " + self.ts_names[q])
            self.tgraph.edges[p, q] = 0
            self.tgraph.edges[q, p] = 0
            p = p+1
            q = q+1
    ...
    def cmi_dsr(self, x, y, z_dict=dict(), p_value=False, k=10, sig_samples=10000):
        ...
        if len(names_z) > 0:
            ...
            z = None
        return indep_test_iid(x, y, z, sig_samples=sig_samples, p_value=p_value, measure="cmiknn", k=k)
...
class CITMI_MISALIGNED:
    ...
    def _cmi_sep_set_pq(self, p, q, set_size):
        ...
        """
        v_list = []
        # r_list = [r for r in range(self.graph.d) if (r != p) and (r != q) and ((self.graph.edges[p, r] != 0) or (self.graph.edges[q, r] != 0))]
        r_list = [r for r in range(self.graph.d) if (r != p) and (r != q) and ((self.graph.edges[p, r] != 0) or (self.graph.edges[q, r] != 0))]
...
if __name__ == "__main__":
    import pandas as pd
    path = "../../../../data/simulated_ts_data/diamond/data_"+str(0)+".csv"
    ...
    ci = PCTMI(data, verbose=True, p_value=False, num_processor=1)
    # ci.fit()
    ...
    print(ar)
    ci.graph.edges = ar
    ar = ci._find_all_open_backdoor_paths(2, 3)
    print(ar)
